REM $Header: v1.2 dashtop.sql 2016/06/22 andy.klock $
--------------------------------------------------------------------------------
--
...
--
-- Modified by Andy Klock to tune for a database with many ASH rows YMMV       
--------------------------------------------------------------------------------
COL "%This" FOR A6
...
--    AND snap_id IN (SELECT snap_id FROM dba_hist_snapshot WHERE sample_time BETWEEN &3 AND &4) -- for partition pruning
    ...
    AND SNAP_ID IN (SELECT snap_id FROM dba_hist_snapshot WHERE begin_interval_time > trunc(&3) AND end_interval_time < &4 + 1/24)
GROUP BY
        &1
    ORDER BY
        "TotalSeconds" DESC
